...
pub fn Saver(comptime Writer: type) type {
    ...
    return struct {
        fn writeHelper(writer: Writer, info: SaveInfo, maybe_data: ?[]const u8) !void {
            ...
            const num_channels = std.math.cast(u16, info.num_channels) orelse 1;
            const sample_rate = std.math.cast(u32, info.sample_rate) orelse 16000;
            ...
            const data_len = if (maybe_data) |data| (std.math.cast(u32, data.len) orelse 0) else 0;
